...
ggplot(fp,aes(x=ageat1stbirth,color=urban,fill=urban))+geom_bar(position = "dodge",color="grey")+facet_zoom(xlim = c(26,34))+scale_x_continuous(breaks=pretty_breaks())+ggtitle("A histogram showing the distribution of age of 1st birth by urban/rural")
```
```{r age_birth va age_married}
ggplot(fp,aes(agefrstmar,ageat1stbirth))+geom_point()
```
